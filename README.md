...
- Из-за ограничений связки **Windows + WSL2 + NTFS** смена владельца на `www-data` (UID 33) неудалась на уровне Docker-контейнера. Владелец остаётся `root`, но контроль доступа обеспечивается **битами прав**, а не именем владельца.
